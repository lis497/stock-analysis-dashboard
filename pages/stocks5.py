...
with sec1:
    ...
    portfolio = pd.DataFrame({
        ...
        "Shares": [default_shares.get(ticker,0) for ticker in selected_tickers],
    })
    #st.write(stocks.get_stock_price(start_date=start_date,end_date=end_date,columns=selected_tickers))
    #df0 = stocks.get_stock_price(start_date=start_date,end_date=end_date,columns=selected_tickers).head(1)
    #portfolio['Investment'] = portfolio['Shares'] * df0
    ...
    #st.write("portfolio_shares")
    #st.write(edited_portfolio["Shares"])
    #st.write("df0.T")
    #st.write(df0.T.reset_index().iloc[:,1])
    #st.write("final result")
    #st.write(df0.T.reset_index().iloc[:,1] * edited_portfolio["Shares"])
    #total = round(sum(df0.T.reset_index().iloc[:,1] * edited_portfolio["Shares"]),2)
    #st.write(edited_portfolio)
    ...
    #get_portfolio is an API, not a RESTAPI
    value, total, total2, profit,change = stocks.get_portfolio(start_date=start_date,end_date=end_date,columns=selected_tickers, shares=edited_portfolio['Shares'])
    
    st.write(f'### Stock Portfolio')
    fig = go.Figure()


    fig.add_trace(go.Pie(
        labels=edited_portfolio["Ticker"],
        values = profit,
        text = change,
        hole = 0.52,
    ...
    fig.add_trace(go.Pie(
        labels=edited_portfolio["Ticker"],
        #values=edited_portfolio['Shares'].astype(float),
        #values = df0.T.reset_index().iloc[:,1] * edited_portfolio["Shares"],
        values=value,
        hole=0.30,
        #marker=dict(colors=["lightgray", profit_color]),
        textinfo="value",
        domain={"x": [0.25, .75], "y": [0.25, .75]},
        sort=False
    ))
    ...
    fig.add_annotation(
        text=f"Pro:{total2}<br>▲<br>Inv:{total}" if total2 > total else f"Pro:{total2}<br>▼<br>Inv:{total}",
        x=.5, y=.5,
        showarrow=False,
        font=dict(size=15)
        )
    ...
    st.write(f'### Stocks Price')
    st.line_chart(stocks.get_stock_price(start_date=start_date,end_date=end_date,columns=selected_tickers))
